...
public class PrayTime {
    ...
    // For calculating Tahajjud
    public int calcTahajjud(int init, int fin) {
        int hoursDiff = (init - fin)/60;
        int diff3 = hoursDiff / 3;
        int tStarting = (fin/60)-diff3;
        return ((init + diff3 + diff3));
    }

    public int toSerial(int hr, int mn) {
        return (hr * 60 + mn);
    }

    public int getHr(int n) {
        return (n / 60);
    }
